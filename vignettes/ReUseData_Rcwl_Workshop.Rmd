...
within _R_ using the `Mutect2` from GATK. This whole workflow is based
on a unified _R_ programming language and can be deployed in local
computer, HPC and cloud computing platforms, using `docker`,
`singularity` or `udocker`. [FIXME] With the pre-built tools in
`RcwlPipelines`, we should be able to conduct reproducible data
analysis using commonly used bioinformatics tools (including
command-line based tools and _R/Bioconductor_ packages) and validated,
best practice workflows (based on workflow languages such as CWL)
within a unified _R_ programming environment.
...
`ReUseData`, as the name suggests, commits to promoting the data
reuse. Data (rom `dataSearch`) can be prepared in standard input
