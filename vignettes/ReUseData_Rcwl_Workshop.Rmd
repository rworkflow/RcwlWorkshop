...
- `dataUpdate`: Update the local data cache for first-time use or if
  any new datasets are generated locally with `getData`. (`dir` works
  recursively to cache all data in sub-directories). 
...
utility functions to retrieve specific information about the data.
...
Data path can be retrieved using `dataPaths` function, which will be
passed directly to the `Rcwl` tool recipe for reproducible data
analysis in _R_.
